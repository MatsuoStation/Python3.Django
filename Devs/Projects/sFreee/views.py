...
from .Util.db_sFreee import DB_sFreee_2107, DB_sFreee_2207, DB_sFreee
	...
	def get_context_data(self, **kwargs):
		...
		# for snpv in SnPsVs:
			...
			''' CSV項目 : / 収支区分, 管理番号, 発生日, 決済期日, 取引先, 勘定科目, 税区分, 金額, 税計算区分, 税額, 備考, 品目, 部門, \
				...
				(AE) 数量, 新単価, 旧単価, 旧金額, 旧税額, 新旧差額, 赤伝, p_code, d_type, r_code, '''
			...
			if pRcode == '現金':
				...
				snp_list.append(['収入', snpv.slip, pMd, '', pName, '売上高', str(aTax), str(aVc), '内税', str(aTc), oCC, pItem, 'SS関係', \
								...
								str(pAm), str(aUc), str(oUc), str(oVl), str(oTax), '', snpv.red_code, snpv.p_code, snpv.d_type, snpv.r_code])
			else:
				BankId = ''
				snp_list.append(['収入', snpv.slip, pMd, '', pName, '売上高', str(aTax), str(aVc), '内税', str(aTc), oCC, pItem, 'SS関係', \
								'', pRcode, '', str(BankId), '', \
...
class GAS_2207(ListView):
	model = SHARPnPOS_2108_2207
	form_class = DateForm
	template_name = 'sFreeeList.html'
	...
	def post(self, request, *args, **kwargs):
		...
		mdate_post = request.POST['mdate']
		if form.is_valid():
			return HttpResponseRedirect('/sFreee/GAS_2207/%s' % mdate_post)
		return render(request, self.template_name, {'form': form})
	...
	def get_context_data(self, **kwargs):
		...
		try:
			...
			## DB : Setup ##
			SnPs, SnP30, SnP40, SnP50, SnP90 = DB_sFreee_2207(self, dlb, dla)
			# SnPs, SnP20, SnP30, SnP40, SnP50, SnP90 = DB_sFreee(self, dlb, dla)
			# SnPs = DB_sFreee(self, dlb, dla)
...
class GAS_2107(ListView):
	...
	def post(self, request, *args, **kwargs):
		...
		mdate_post = request.POST['mdate']
		if form.is_valid():
			return HttpResponseRedirect('/sFreee/GAS_2107/%s' % mdate_post)
		return render(request, self.template_name, {'form': form})
	...
	def get_context_data(self, **kwargs):
		...
		try:
			...
			## DB : Setup ##
			SnPs, SnP30, SnP40, SnP50, SnP90 = DB_sFreee_2107(self, dlb, dla)
			# SnPs, SnP20, SnP30, SnP40, SnP50, SnP90 = DB_sFreee(self, dlb, dla)
			# SnPs = DB_sFreee(self, dlb, dla)
